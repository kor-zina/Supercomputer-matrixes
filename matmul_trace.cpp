...
matrix<double> matmul_ikj_(int *sendcounts, int *displs, matrix<double> a, matrix<double> b, MPI_Comm comm)
{
    int rowa = a.nrows();
    int rowb = b.nrows();
    int cola = a.ncols();
...
double trace(matrix<double> a, MPI_Comm comm)
{
    int myrank, comm_size;
    MPI_Comm_size(comm, &comm_size);
    MPI_Comm_rank(comm, &myrank);
    ...
    double tr_sum = 0;
    MPI_Reduce(&tr, &tr_sum, 1, MPI_DOUBLE, MPI_SUM, 0, comm);
    return tr_sum;
}
...
int main(int argc, char *argv[])
{
    ...
    matrix<double> c = matmul_ikj_(sendcounts, displs, a, b, MPI_COMM_WORLD);
    double tr = trace(c, MPI_COMM_WORLD);
